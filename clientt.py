...
class FTXClient:
    ...
    def modify_order(
        ...
        assert (existing_order_id is None) ^ (existing_client_order_id is None),             'Must supply exactly one ID for the order to modify'
    ...
    def get_coin(self, coin: str) -> dict:
        return self._get('wallet/balances',{'coin': coin})
    ...
    def get_quote(self, fromCoin: str, toCoin: str, size: float) -> dict:
    ...
    def get_quote_status(self, quoteId: int = None) -> int:
        return self._get(f'otc/quotes/{quoteId}')
    
    def accept_quote(self, quoteId: int = None) -> dict:
